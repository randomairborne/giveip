...
async fn handle(addr: SocketAddr, req: Request<Body>) -> Result<Response<Body>, Infallible> {
    ...
    let body = match accept.find("text/html") {
        Some(_) => {
            format!(
                r#"<title>Your IP is {0}</title><h1 style="font-size: 32px;font-size: 3vw;height: 75%;width: 100%;display: flex;position: fixed;align-items: center;justify-content: center;">Your public IP is {0}</h1>"#,
                pretty_addr
            )
        }
        None => format!("{}\n", pretty_addr),
